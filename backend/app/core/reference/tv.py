...
SOURCE_BASE_DIR = os.path.dirname(os.path.abspath(__file__))
RUNTIME_BASE_DIR = os.getenv('CMCC_IPTV_RUNTIME_DIR', SOURCE_BASE_DIR)
OUTPUT_DIR = os.path.join(RUNTIME_BASE_DIR, 'output')
CONFIG_DIR = os.path.join(RUNTIME_BASE_DIR, 'config')
LOG_DIR = os.path.join(RUNTIME_BASE_DIR, 'log')
CACHE_DIR = os.path.join(RUNTIME_BASE_DIR, 'cache')

checker_dir = os.path.join(SOURCE_BASE_DIR, 'iptv_checker_v3')
...
# 是否优先使用 hwurl (Huawei)
# True  = 优先提取 hwurl (默认，如果 hwurl 为空则回退到 zteurl)
# False = 优先提取 zteurl (如果 zteurl 为空则回退到 hwurl)
...
# 输出文件名配置
TV_M3U_FILENAME = os.path.join(OUTPUT_DIR, "tv.m3u")        # 组播地址列表文件
TV2_M3U_FILENAME = os.path.join(OUTPUT_DIR, "tv2.m3u")      # 转单播地址列表文件
KU9_M3U_FILENAME = os.path.join(OUTPUT_DIR, "ku9.m3u")      # KU9回看参数格式文件
APTV_M3U_FILENAME = os.path.join(OUTPUT_DIR, "aptv.m3u")    # APTV回看参数格式文件 (新增)
XML_FILENAME = os.path.join(OUTPUT_DIR, "t.xml")            # XML节目单文件
...
def initialize_environment():
    ...
    EXTERNAL_M3U_CACHE_FILE = os.path.join(CACHE_DIR, CACHE_M3U_FILENAME or "cache.m3u")
